// Also handle timeupdate event to detect when the video has played sufficiently
let timeChecked = false;
video_element.addEventListener('timeupdate', function() {
  // If we've played at least 2 seconds and haven't already checked
  if (video_element.currentTime > 2 && !timeChecked) {
    timeChecked = true;
    console.log("Sufficient time has elapsed");
    videoPlayed = true;
    
    // Enable the button if not required or if text is entered
    ...
    if (!trial.required || (trial.required && textArea.value.trim() !== '')) {
      ...
      console.log("Button enabled by timeupdate");
    }
  }
});/**
...
video_element.addEventListener('ended', function() {
  ...
  if (!trial.required || (trial.required && textArea.value.trim() !== '')) {
    ...
    console.log("Text required but not entered yet");
  }
});
