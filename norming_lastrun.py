...
expName = 'norming'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\flani\\Experiment\\norming_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    movie = visual.MovieStim(
        win, name='movie',
        filename='stimuli/initials/batent1.mp4', movieLib='ffpyplayer',
        loop=False, volume=1.0, noAudio=False,
        pos=(0, .25), size=(0.4, 0.4), units=win.units,
    ...
    textbox = visual.TextBox2(
         ...
         ori=0.0, pos=(0, -.25), draggable=False,      letterHeight=0.05,
         size=(0.7, 0.4), borderWidth=2.0,
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('stimuli/initials/loopTemplate1.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[movie, enter, textbox, replay],
        )
        ...
        _enter_allKeys = []
        textbox.reset()
        textbox.setPlaceholder('Describe what is happening in the video above.')
        ...
        while continueRoutine:
            ...
            if enter.status == NOT_STARTED and movie.status==FINISHED:
                # keep track of start time/frame for later
            ...
            if textbox.status == NOT_STARTED and movie.status==FINISHED:
                # keep track of start time/frame for later
            ...
            if replay.status == NOT_STARTED and movie.status==FINISHED:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'replay.started')
                # update status
                replay.status = STARTED
                replay.setAutoDraw(True)
                replay.play()
            
            # if replay is stopping this frame...
            if replay.status == STARTED:
                if bool(enter.status==FINISHED) or replay.isFinished:
                    # keep track of stop time/frame for later
                    replay.tStop = t  # not accounting for scr refresh
                    replay.tStopRefresh = tThisFlipGlobal  # on global time
                    replay.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'replay.stopped')
                    # update status
                    replay.status = FINISHED
                    replay.setAutoDraw(False)
                    replay.stop()
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer], 
                    playbackComponents=[movie, replay]
                )
        ...
        replay.stop()  # ensure movie has stopped at end of Routine
